...
async function connectCosmos(): Promise<StargateClient> {
  let client: StargateClient | undefined;
  await waitFor(
    async () => {
      try {
        client = await StargateClient.connect(COSMOS_RPC);
        return true;
      } catch {
        return false;
      }
  ...
  if (!client) throw new Error("Cosmos RPC became ready without a client");
  return client;
}
...
async function main(): Promise<void> {
  ...
  const cosmos = await connectCosmos();
  ...
  try {
    ...
    await waitFor(
      async () =>
        BigInt((await cosmos.getBalance(account.address, "usei")).amount) > 0n,
