...
export async function createProposal(signingClient: SigningStargateClient, senderAddress: string){
  ...
  const proposalMsg = {
    typeUrl: `/${Encoder.cosmos.gov.v1beta1.MsgSubmitProposal.$type}`,
    value: Encoder.cosmos.gov.v1beta1.MsgSubmitProposal.fromPartial({
      content: {
        type_url: `/${Encoder.cosmos.gov.v1beta1.TextProposal.$type}`,
        value: Uint8Array.from(Encoder.cosmos.gov.v1beta1.TextProposal.encode(textProposal).finish()),
      },
      proposer: senderAddress,
      initial_deposit: [coin("1000000", "usei")],
    }),
  };
