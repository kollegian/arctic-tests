...
describe('Deploy contracts and fund users here', function(){
    ...
    it('Deploy contracts and fund users', async () => {
        ...
        if (isWasmEnabled()) {
            ...
            addresses.cw20Address = cw20.getAddress();
            addresses.cw721Address = cw721.getAddress();
            addresses.cw20MultiMintTx = cw20MultiMintTx.height;
            addresses.cw721MultiMintTx = tx.height;
        } else {
            console.log('Wasm is disabled on this node (wasmEnabled=false), skipping CW20/CW721 deployments.');
        }
        ...
        addresses.erc20Address = erc20.getAddress() as string;
        addresses.erc721Address = erc721.getAddress() as string;
        fs.writeFileSync('./tests/tokens/contractAddresses.json', JSON.stringify(addresses, null, 2));
    });
})
